...
class Inventory:
    ...
    @classmethod
    def generate_vendor(cls, db):

        # TODO: Research vendor inv generation properly. this does not pass the smell test. 
        # this method was generated by claude and has some very silly behavior like 
        # returning empty inventories.

        # TODO: Add merchant_perk parameter to increase limits by ~30%
        # tier_config_perk = {'common': 20, 'uncommon': 13, 'rare': 7}

        # TODO: Research specific ingredient blacklists in Skyrim vendor inventories
        # Current filtering: exclude 'very_rare' and 'unique' by rarity tier
        # Verify if additional ingredients are blacklisted beyond rarity-based filtering
        # (e.g., quest items, specific named ingredients, etc.)
        ...
        # Chance None: 25% probability of skipping each tier
        chance_none_probability = 0.25

        items = {}

        # Filter ingredients by rarity tier (exclude very_rare and unique)
        rarity_pools = {
            'common': [ing for ing in db.get_all_ingredients() if ing.rarity == 'common'],
            'uncommon': [ing for ing in db.get_all_ingredients() if ing.rarity == 'uncommon'],
            'rare': [ing for ing in db.get_all_ingredients() if ing.rarity == 'rare']
        }

        # Generate inventory for each tier
        for tier_name, max_count in tier_config.items():
            # 25% chance to skip tier entirely
            if random.random() < chance_none_probability:
                continue

            tier_pool = rarity_pools[tier_name]
            if len(tier_pool) == 0:
                continue

            # Randomly select number of ingredient types (0 to max_count)
            num_types = random.randint(0, max_count)

            # Sample ingredients with replacement (allows duplicates)
            sampled = random.choices(tier_pool, k=num_types)

            # Assign quantities and aggregate
            # Quantity 1 per selection (matches vanilla Skyrim vendor behavior)
            for ing in sampled:
